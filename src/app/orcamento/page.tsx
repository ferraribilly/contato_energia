...
const OrcamentoPage: React.FC = () => {
  ...
  useEffect(() => {
    if (cep.length === 8) {
      axios
        .get(`https://www.portalsolar.com.br/api/v1/simulations/power_distributors?postalcode=${cep}`)
        .then(response => {
  ...
  return (
    <div
      ...
      style={{ backgroundImage: "url('/casa_solar.png')" }} // recomendado colocar a imagem na pasta public
    >
          ...
          {etapa === 1 && (
            <>
              ...
              <input
                ...
                value={cep}
                maxLength={8}
                onChange={e => setCep(e.target.value.replace(/\D/g, ''))} // só números
              />
              <button
          ...
          {etapa === 2 && (
            <>
              ...
              <p className="text-gray-700 text-sm mt-3">
                Concessionária: <span className="font-bold">{concessionaria}</span>
              </p>
              <button
                className="bg-orange-500 text-white w-full p-2 rounded mt-4 hover:bg-orange-600"
                onClick={() => {
            ...
            <div className="text-center">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="bg-white p-4 rounded-lg shadow-md text-center">
                  ...
                  <p className="text-xl font-bold">
                    {(custoEstimado * 1.2).toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })}
                  </p>
                </div>
                <div className="bg-white p-4 rounded-lg shadow-md text-center">
                  <FaSolarPanel className="text-yellow-500 text-3xl mx-auto mb-2" />
                  <p className="text-gray-600">Produção Mensal</p>
                  <p className="text-xl font-bold">{producaoMensal} kWh</p>
                </div>
                <div className="bg-white p-4 rounded-lg shadow-md text-center">
              ...
              <button
                className="bg-orange-500 text-white w-full p-3 rounded mt-6 hover:bg-orange-600"
                onClick={() => setEtapa(1)}
              >
                Refazer Simulação
              </button>
